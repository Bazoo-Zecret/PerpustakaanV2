...
public function ubah_siswa($id,$nis,$nama,$kelas,$gambar)
{
    if($gambar == null){
        $query = $this->koneksi->query("UPDATE siswa SET nis='$nis',nama='$nama',kelas='$kelas' WHERE siswa_id='$id'");
        session_start();
        $_SESSION['success'] = 'Data siswa berhasil di ubah';
        header("location:../dashboard.php?pages=siswa");
    } else {
        move_uploaded_file($_FILES['photo']['tmp_name'],'../assets/img/siswa/'. $gambar);
        $query = $this->koneksi->query("UPDATE siswa SET nisn='$nis',nama_siswa='$nama',kelas='$kelas',photo='$gambar' WHERE siswa_id='$id'");
        session_start();
        $_SESSION['success'] = 'Data siswa berhasil di ubah';
        header("location:../dashboard.php?pages=siswa");
    }
